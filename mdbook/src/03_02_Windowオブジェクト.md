...
JavaScriptの初心者が混乱しやすい点の一つが、オブジェクトと `const` の意外な関係です。
...
「強力なガムテープで名札が貼られた箱」をイメージしてください。ガムテープで固定されているので、名札を別の新しい箱に貼り直すことはできません。でも、箱の蓋（ふた）を一つ開けて、中に入っているお菓子を入れ替えたり、ジャムを塗ったりすることは自由なのです。

<ruby>例<rt>たと</rt></ruby>えるなら、「ファイルの保存場所（パス）」は固定されているけれど、「ファイルの中身」は自由に書き換えられるようなイメージです。

先ほどの<ruby>例<rt>れい</rt></ruby>では、`const`を使って宣言した`message`という箱を開けて、中身を"こんにちは、日本へようこそ！"に入れ替えました。これはエラーにはなりません。
