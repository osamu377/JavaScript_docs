...
> 自分で作った<ruby>変数<rt>へんすう</rt></ruby>も、実は `window` の持ち物になることがあります。
...
* **`document.title`**
ブラウザのタブに<ruby>表示<rt>ひょうじ</rt></ruby>されている「ページのタイトル」です。JavaScriptで書き換えることもできます。
* **`document.body`**
...
<ruby>設定<rt>せってい</rt></ruby>されているスタイルの値を取得します。
...
* **`src`**（ソース）
<ruby>画像<rt>がぞう</rt></ruby>の「ファイルの場所（URL）」です。ここを書き換えると、別の<ruby>画像<rt>がぞう</rt></ruby>に切り替わります。
* **`width` / `height**`
...
button.addEventListener("click", function(event) {
  // この「event」という<ruby><rt>引数</rt>ひきすう</ruby>が、Eventオブジェクト（報告書）です
  console.log(event); 
});
...
* **よく使う場所：** フォームの<ruby>入力<rt>にゅうりょく</rt></ruby>が間違っているとき、送信を止めるために使います。
