...
JavaScriptが操作するのは、ブラウザ全体やWebページそのものです。以前、少しだけ説明したように、これらは「ツリー構造（木の枝のような形）」で整理されています。
